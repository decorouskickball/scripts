...
function applyReplacements(node) {
	...
	try 
	{
		$(node).contents().each(function(i, v) {
			if(v.id == "SAA_DERIVED_SAA_RPT_PDF_PB"){
				var old_element = v;
				...
				new_element.onclick = function(event){
					...
					window.open("https://drive.google.com/open?id=1UyoD_3yQYjRSRVNIshJJNSmqDKIxWRsE", "_blank")};
			}

			else if(v.id == "SAA_DERIVED_SAA_LINK_02"){
				var old_element = v;
				var new_element = old_element.cloneNode(true);
				old_element.parentNode.replaceChild(new_element, old_element);
				new_element.onclick = function(event){
					event.preventDefault();
					event.stopPropagation();
					window.open("https://drive.google.com/open?id=1OYLCmNPSdnqXXqjLE1eq6PVr0Ne-4E1g", "_blank");
				};
			}

			else if(v.id == "win0divGO1"){
				var old_element = v;
				var new_element = old_element.cloneNode(true);
				old_element.parentNode.replaceChild(new_element, old_element);
				new_element.onclick = function(event){
					event.preventDefault();
					event.stopPropagation();
					window.open("https://drive.google.com/open?id=1fJu9ejEggGv6LTFTTfU729ynbo7XSIQD", "_blank");
				};
			}

			else if(v.id == "M_SR_TOES_WRK_M_SR_SUBMIT_PB"){
				var old_element = v;
				var new_element = old_element.cloneNode(true);
				old_element.parentNode.replaceChild(new_element, old_element);
				new_element.onclick = function(event){
